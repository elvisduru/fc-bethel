...
const { Contact, Subscriber, Partnership, Blog, Gallery } = require('./db/models')
...
app.route('/api/blog')
  ...
  .post(async (req, res) => {
    try {
      const form = formidable({ uploadDir: path.join(__dirname, '/uploads'), keepExtentions: true })
      let newFileName;
      form
        ...
        .parse(req, async (err, fields, file) => {
          ...
          res.status(201).json({ msg: "Post has been published successfully!", url: blog.slug })
        })
    } catch (e) {
      console.log(e)
    }
    ...
    try {
      ...
      form
        ...
        .parse(req, async (err, fields, file) => {
          ...
          if (file.image) {
            updatedBlog.image !== "undefined" && fs.unlinkSync(path.join(__dirname, updatedBlog.image))
            blog.image = '/uploads/' + newFileName
          } else {
          ...
          res.status(201).json({ msg: "Post has been updated successfully!", url: blog.slug })
        })
    } catch (e) {
      console.log(e)
    }
...
app.route('/api/gallery')
  ...
  .post(async (req, res) => {
    try {
      const form = formidable({ uploadDir: path.join(__dirname, '/uploads/gallery'), keepExtentions: true })
      let uploads = [], newFileName;
      form
        .parse(req)
        .on('fileBegin', (name, file) => {
          newFileName = new Date().getTime() + file.name;
